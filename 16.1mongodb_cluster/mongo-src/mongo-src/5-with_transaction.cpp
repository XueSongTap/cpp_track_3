...
//
// g++ --std=c++17 5-with_transaction.cpp -o 5-with_transaction $(pkg-config --cflags --libs libmongocxx)
int main() {
    ...
    // uriString = 'mongodb://mongos0.example.com:27017,mongos1.example.com:27017/'
     // 连接复制集集群   
    mongocxx::client client{mongocxx::uri{"mongodb://localhost:28017,localhost:28018,localhost:28019/?replicaSet=rs0"}};

    write_concern wc_majority{};
    wc_majority.acknowledge_level(write_concern::level::k_majority);        // 配置写关注

    read_concern rc_local{};
    rc_local.acknowledge_level(read_concern::level::k_local);   // 配置读关注

    read_preference rp_primary{};
    rp_primary.mode(read_preference::read_mode::k_primary);     // 配置读偏好

    // Prereq: Create collections.

    auto foo = client["mydb1"]["foo"]; // 操作两个库的不同集合
    ...
    std::cout << "An transaction ok" << std::endl;
    return EXIT_SUCCESS;
