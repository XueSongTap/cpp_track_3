...
static int udp_server_entry(__attribute__((unused))  void *arg) {
	...
	uint8_t buffer[BUF_SIZE];
  	...
  	sock = nsocket(AF_INET, SOCK_DGRAM, 0);

  	rc = nbind(sock, (struct sockaddr*) &addr, addr_len);
    ...
    /* Receive DNS query */
    	nbytes = nrecvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr *) &client_addr, &addr_len);

		//printf("dns server --> buffer: %s\n", buffer);
	...
	//printf("nbytes : %d, buflen : %d\n", nbytes, buflen);
    	nsendto(sock, buffer, buflen, 0, (struct sockaddr*) &client_addr, addr_len);
  }
...
static int tcp_server_entry(__attribute__((unused))  void *arg)  {
	...
	printf("bsp arno, malixiaochaoren , zsk, close eye\n");
