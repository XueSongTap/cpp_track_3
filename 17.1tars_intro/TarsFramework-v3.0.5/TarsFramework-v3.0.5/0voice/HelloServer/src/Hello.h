...
namespace TestApp
{
    ...
    public:
        int onDispatch(tars::TarsCurrentPtr _current, vector<char> &_sResponseBuffer) // _sResponseBuffer 返回的数据
        {
            ...
            switch(r.first - __TestApp__Hello_all)
            {
                ...
                case 1:  // testHello
                {
                    ...
                    tars::Int32 _ret = testHello(sReq,sRsp, _current);  // 调用业务层的testHello
                    if(_current->isResponse())
                    {
                        if (_current->getRequestVersion() == TUPVERSION)
                        {
